...
var exprVer = 'v2'
try { var _ev = localStorage.getItem('dsh-whale-expr-ver'); if (_ev === 'v1' || _ev === 'v2' || _ev === 'v3') exprVer = _ev } catch (err) {}
...
function exprSelectOptions() {
  ...
  var list = (exprVer === 'v1') ? EXPR_MENU_V1 : (exprVer === 'v3') ? EXPR_MENU_V3 : EXPR_MENU_V2
...
function exprMoodImg(name) {
  // 各版本的情绪图名不同：v1/v2 用同一套英文姿态名，v3 是第三版素材
  if (exprVer === 'v3') {
    if (name === 'angry') return 'hm'        // 连点生气 → 哼哼哼
...
function exprApplyIcon() {
  ...
  if (manualExpr) {
    if (pressing) { exprSetImg(EXPR_URL(exprVer === 'v3' ? 'hit' : 'close_eyes')); return }
...
function pressDown() {
  ...
  pressing = true
  if (mood === 'normal' && !exhaustedMode) exprSetImg(EXPR_URL(exprVer === 'v1' ? 'stroking' : (exprVer === 'v3' ? 'hit' : 'close_eyes')))
  playPress()
}
function pressUp() {
...
function apply(ctx) {
      ...
      handler: (req, res) => {
        try {
          ...
          try {
            ...
            const ver = (verRaw === 'v1' || verRaw === 'v2' || verRaw === 'v3') ? verRaw : 'v2'
    ...
    const EXPR_VER_DIRS = { v1: 'v1', v2: 'v2', v3: 'v3' }
