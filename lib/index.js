...
var exprVer = 'v2'
try { var _ev = localStorage.getItem('dsh-whale-expr-ver'); if (_ev === 'v1' || _ev === 'v2') exprVer = _ev } catch (err) {}
...
img.alt = 'DeepSeek 余额'
img.draggable = false
// 图片加载失败：清除去重标记，下次 exprSetImg 自动重试（防止破图卡死）
img.addEventListener('error', function () {
  __curImgSrc = ''
  // 延迟 1.5 秒自动重试当前版本主图/表情
  setTimeout(function () {
    if (__curImgSrc === '' && img && img.src) {
      var cur = img.src
      // 用原 URL 重设触发浏览器重试（绕过 exprSetImg 去重——已清空）
      img.src = cur
    }
  }, 1500)
})
...
function exprSelectOptions() {
  ...
  var list = (exprVer === 'v1') ? EXPR_MENU_V1 : EXPR_MENU_V2
...
function exprApplyIcon() {
  ...
  if (manualExpr) {
    if (pressing) { exprSetImg(EXPR_URL('close_eyes')); return }
    exprSetImg(EXPR_URL(manualExpr))
    return
  }
  if (mood === 'angry') { exprSetImg(EXPR_URL('angry')); return }
  if (mood === 'disappointed') { exprSetImg(EXPR_URL('disappointed')); return }
  if (mood === 'shy') { exprSetImg(EXPR_URL('shy')); return }
  if (exhaustedMode) { exprSetImg(EXPR_URL('exhausted')); return }
  if (pressing) { exprSetImg(EXPR_URL('close_eyes')); return }
  exprSetImg(IMG_URL())
}
...
function exprEnterDisappointed() {
  ...
  exprCancelBlink()
  exprClearTimers()
  exprSetImg(EXPR_URL('disappointed'))
...
// 表情图加载就绪标记（onload 置 true；用于眨眼/动画切帧前确认缓存可用）
var __exprReady = {}
var __exprReadyImgs = {}  // 持有引用防 GC：无引用的 Image 会被回收，加载被取消（st:0）
function exprImgReady(name) {
  if (__exprReady[name]) return true
  if (__exprReadyImgs[name]) return false  // 已在加载中，别重复发请求
  var im = new Image()
  __exprReadyImgs[name] = im
  im.onload = function () { __exprReady[name] = true; delete __exprReadyImgs[name] }
  im.onerror = function () { delete __exprReadyImgs[name] }  // 失败允许下轮重试
  im.src = EXPR_URL(name)
  return false
}
...
function buildGroup1() {
  if (selectedAccountIdx === -1) {
    ...
    return [
      ...
      { t: '当前余额 ' + fmt(state.balance, state.currency), s: 'C', c: '' },
    ]
  }
...
function apply(ctx) {
      ...
      handler: (req, res) => {
        try {
          ...
          try {
            ...
            const ver = (verRaw === 'v1' || verRaw === 'v2') ? verRaw : 'v2'
    ...
    const EXPR_VER_DIRS = { v1: 'v1', v2: 'v2' }
