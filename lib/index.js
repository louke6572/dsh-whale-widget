...
function hideBubble() {
  ...
  var _wasManualLine = !!manualLine
  if (manualLine) manualLine = null
  // 手动台词气泡关闭后立即恢复额度三行（否则文字停住在台词，
  // 用户看到的就是"额度显示坏了、只有一句台词"）。只在气泡淡出后恢复，
  // 且要避开消耗金额泡泡（showCostBubble 自己管理文字）。
  if (_wasManualLine && !costBubbleActive && manualExpr === null) {
    setTimeout(function () {
      // 气泡已隐藏，恢复过程不可见
      restoreBubbleLines()
    }, 260)
