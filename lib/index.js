...
function buildGroup1() {
  if (selectedAccountIdx === -1) {
    ...
    // 气泡只有三行槽位：峰谷时段 / 今日消耗 / 当前余额
    // 今日消耗优先取本地 last-turn 逐轮累计（真实反映 dsh 对话消耗）；
    // state.todayUsage 是平台侧记账，DeepSeek 长期不用时恒为 0 无意义
    var todayNum = todayCost > 0 ? todayCost : (state.todayUsage != null ? Number(state.todayUsage) : 0)
    var todayTxt = todayNum.toFixed(2)
    return [
      { t: (peak ? peakText : offText) + ' · 今日消耗 ¥' + todayTxt, s: 'A', c: '', w: true },
      { t: fmt(state.balance, state.currency), s: 'B', c: '' },
      { t: peak ? '高峰计价 · 比空闲贵一倍' : '空闲计价 · 半价优惠中', s: 'C', c: peak ? '#e0433f' : '#2fa24c' },
    ]
  }
